...
from helpers import login_required, credentials_to_database, credentials_to_dict, update_credentials, best_times, list_to_string, best_times_allday, get_timezone, find_conflicts, find_conflicts_allday, fill, dst_start, dst_end, format_date, format_date_readable
    ...
    if request.method == "POST":
        ...
        if request.form.get("allday") == None:
            try:
                ...
                start_time = datetime.time.fromisoformat(str(int(request.form.get("start_time_hours")) + int(request.form.get("start_time_noon"))
                                 ).zfill(2) + ":" + request.form.get("start_time_minutes").zfill(2))
                end_time = datetime.time.fromisoformat(str(int(request.form.get("end_time_hours")) + int(request.form.get("end_time_noon"))
            ...
            if start_time >= end_time:
                return render_template("apology.html", message="Start time cannot be after or equal to end time.")
        ...
        return render_template("created.html", ID=event_id, PASSWORD=request.form.get("password"))
